...
def format_display_date(date_str):
    """Convert YYYY-MM-DD to 'Monday, February 23, 2026' format"""
    if not date_str:
        return ""
    try:
...
# Custom CSS for basketball court styling
st.markdown("""
<style>
    /* Sidebar - Wood grain texture */
    [data-testid="stSidebar"] {
        background: 
            linear-gradient(rgba(240, 242, 246, 0.85), rgba(240, 242, 246, 0.85)),
    ...
    /* Main app - white in light mode only */
    ...
    /* Style the header area with dark background */
    [data-testid="stHeader"] {
        background: linear-gradient(90deg, #262730 0%, #1a1a24 100%) !important;
    ...
    .main h1:first-of-type {
        background: linear-gradient(90deg, #262730 0%, #1a1a24 100%);
        color: white !important;
        padding: 25px 30px;
    ...
    /* #1 & #6: Rounded corners, shadows, and alternating rows on tables */
    ...
    /* #1: Better header styling */
    [data-testid="stDataFrame"] thead tr {
        background: linear-gradient(135deg, #262730 0%, #1a1a24 100%) !important;
    }
    ...
    /* #7: Enhanced alternating row colors (subtle) - light mode only */
    @media (prefers-color-scheme: light) {
    ...
    /* #3: Column alignment - center numbers, left-align text */
    ...
    /* #4: Subtle borders between columns */
    ...
    /* #4: Loading states - Basketball spinner animation */
    [data-testid="stSpinner"] > div {
        border-color: rgba(255, 107, 53, 0.2) !important;
        border-top-color: #FF6B35 !important;
    }
    
    /* Add basketball emoji to loading spinner */
    ...
    @media (max-width: 768px) {
        ...
        /* Smaller padding in table cells on mobile */
        [data-testid="stDataFrame"] tbody td,
        [data-testid="stDataFrame"] thead th {
    ...
    /* Primary buttons (like Submit Picks) */
    .stButton > button[kind="primary"] {
        background: linear-gradient(135deg, #FF6B35 0%, #F7931E 100%) !important;
        box-shadow: 0 4px 14px rgba(255, 107, 53, 0.4) !important;
    }
    
    /* #9: Tab styling improvements */
    .stTabs [data-baseweb="tab-list"] {
        gap: 8px;
    ...
    .stTabs [data-baseweb="tab"] {
        ...
        padding: 10px 20px;
        background-color: var(--tab-bg, #f0f2f6);
    ...
    .stTabs [data-baseweb="tab"]:hover {
        background-color: var(--tab-hover-bg, #e0e2e6);
    }

    @media (prefers-color-scheme: light) {
        :root { --tab-bg: #f0f2f6; --tab-hover-bg: #e0e2e6; }
    }
    @media (prefers-color-scheme: dark) {
        :root { --tab-bg: #1e2130; --tab-hover-bg: #2a2f45; }
        .stTabs [data-baseweb="tab"] { color: #d0d0d0 !important; }
    }
    ...
    @media (prefers-color-scheme: dark) {
        ...
        /* Table cell text colors - override the light-mode greens/reds that go invisible */
        [data-testid="stDataFrame"] tbody td:nth-child(2) {
        ...
        div[style*="border-top"] {
            border-top-color: #2a2f45 !important;
            color: #9ca3af !important;
        }
    }
</style>
...
if 'show_champion_standings' not in st.session_state:
    st.session_state.show_champion_standings = False
...
if not st.session_state.logged_in:
    ...
    with st.sidebar:
        ...
        for season_name in sorted(data['seasons'].keys(), reverse=True):
            ...
            is_upcoming = s.get('upcoming', False)
            if is_upcoming:
                label = f"📅 {season_name} (Upcoming)"
            else:
                label = f"📅 {season_name}"
                if is_current:
                    label += " (Current)"
        ...
        # Past Champions section — show any locked season with a champion
        champion_seasons = sorted(
            [s for s in data['seasons'].keys() 
             if data['seasons'][s].get('locked') or data['seasons'][s].get('champion')
             or (s != current_season and not data['seasons'][s].get('upcoming', False))],
            reverse=True
        )
        # Filter to only seasons that are locked OR have a champion stored, plus upcoming stubs
        champion_seasons = sorted(
            [s for s in data['seasons'].keys()
             if data['seasons'][s].get('locked') 
             or data['seasons'][s].get('champion')
             or data['seasons'][s].get('upcoming', False)],
            reverse=True
        )
        if champion_seasons:
            st.write("---")
            st.write("**🏆 Past Champions:**")
            for past_season in champion_seasons:
                s_info = data['seasons'][past_season]
                champion = s_info.get('champion')
                if not champion:
                    try:
                        s_df = get_season_standings(data, past_season)
                        if not s_df.empty:
                            champion = s_df.iloc[0]['Name']
                    except:
                        champion = None
                is_upcoming = s_info.get('upcoming', False)
                if is_upcoming:
                    st.caption(f"📅 {past_season}: *Upcoming*")
                elif champion:
                    btn_label = f"🥇 {past_season}: {champion}"
                    if st.button(btn_label, key=f"champ_btn_{past_season}", use_container_width=True):
                        st.session_state.viewing_season = past_season
                        st.session_state.show_champion_standings = True
                        st.rerun()
                else:
                    st.caption(f"📅 {past_season}: *No data*")
    ...
    # Champion standings overlay — shown when navigating from Past Champions sidebar
    if st.session_state.get('show_champion_standings') and is_viewing_past:
        champ_season = viewing_season
        champ_info = data['seasons'].get(champ_season, {})
        champion = champ_info.get('champion')
        if not champion:
            try:
                tmp_df = get_season_standings(data, champ_season)
                if not tmp_df.empty:
                    champion = tmp_df.iloc[0]['Name']
            except:
                champion = None

        st.markdown(f"## 🏆 {champ_season} Season — Final Standings")
        if champion:
            st.success(f"🥇 Champion: **{champion}**")
        
        champ_df = get_season_standings(data, champ_season)
        if not champ_df.empty:
            display_df = champ_df[['Name', 'Wins', 'Losses', 'Confidence']].rename(columns={'Confidence': 'Conf Pts'})
            table_height = 38 + len(display_df) * 35 + 2
            st.dataframe(display_df, use_container_width=True, hide_index=False, height=table_height)
        
        col1, col2 = st.columns([1, 5])
        with col1:
            if st.button("↩️ Back to Current Season", key="champ_back"):
                st.session_state.show_champion_standings = False
                st.session_state.viewing_season = None
                st.rerun()
        st.divider()
    ...
    if st.session_state.user_type == "admin" or st.session_state.is_admin:
            ...
            if str(week_num) in season_data.get('weeks', {}) and season_data['weeks'][str(week_num)]['games']:
                ...
                for game in week_data['games']:
                    ...
                    with col1:
                        ...
                        caption_parts = []
                        if game.get('date'):
                            caption_parts.append(format_display_date(game['date']))
                        if is_neutral:
        ...
        with tabs[2]:
            ...
            if active_users:
                ...
                if str(week_num) in season_data.get('weeks', {}) and season_data['weeks'][str(week_num)]['games']:
                    ...
                    # Show submission status FIRST (persists after save)
                    if user_picks and user_picks.get('submitted'):
                        submitted_time = user_picks.get('submitted', '')
                        try:
                            dt = datetime.fromisoformat(submitted_time)
                            formatted_time = dt.strftime("%B %d, %Y at %I:%M %p")
                        except:
                            formatted_time = submitted_time
                        st.success(f"✅ Picks already submitted for {selected_display} - Week {week_num} on {formatted_time}")
                    
                    # Show one-time success message after save
                    if 'edit_picks_saved' in st.session_state:
                        st.success(f"🎉 Picks just saved for {st.session_state.edit_picks_saved}!")
                        st.balloons()
                        del st.session_state.edit_picks_saved
                    
                    # Info box about confidence picks
                    st.info("📌 **Confidence Picks:** Select exactly 3 games and assign confidence values (3 = most confident, 2 = medium, 1 = least). Each value (1, 2, 3) can only be used once.")
                    ...
                    for i, game in enumerate(week_data['games']):
                        ...
                        with col2:
                            ...
                            pick = st.radio(
                                f"Pick:",
                                [game['away'], game['home']],
                                key=f"edit_pick_{week_num}_{game['id']}_{selected_user}",  # Add user to key to force refresh
                                horizontal=True,
                                index=default_pick
                            )
                            picks[game['id']] = pick
                        ...
                        with col3:
                            # Get currently used confidence values (from previous games in loop)
                            used_values = [str(v) for gid, v in confidence_picks]
                            
                            # Build available options (exclude already used)
                            available_options = ["None"]
                            for val in ["3", "2", "1"]:
                                if val not in used_values:
                                    available_options.append(val)
                            
                            # Get default value for this game
                            default_conf = "None"
                            if user_picks:
                                for conf_game_id, conf_val in user_picks.get('confidence', []):
                                    if conf_game_id == game['id']:
                                        default_conf = str(conf_val)
                                        # If this is a previously saved value, make sure it's in available options
                                        if default_conf not in available_options and default_conf != "None":
                                            available_options.append(default_conf)
                            
                            # Get index safely
                            try:
                                conf_index = available_options.index(default_conf)
                            except ValueError:
                                conf_index = 0  # Default to "None"
                            
                            confidence = st.selectbox(
                                "Confidence:",
                                available_options,
                                key=f"edit_conf_{week_num}_{game['id']}_{selected_user}",  # Add user to key to force refresh
                                index=conf_index
                            )
                            if confidence != "None":
                    ...
                    if st.button("💾 Save Picks", key="save_edited_picks", type="primary"):
                        if len(confidence_picks) == 3 and len(set(conf_values)) == 3:
                            ...
                            save_data(data)
                            # Store success message in session state so it survives rerun
                            st.session_state.edit_picks_saved = f"{selected_display} - Week {week_num}"
                            # Force rerun to show success message at top
                            st.rerun()
                        else:
        ...
        with tabs[3]:
            ...
            if str(week_num) in season_data.get('weeks', {}) and season_data['weeks'][str(week_num)]['games']:
                ...
                for game in games:
                    ...
                    row = {
                        'Game': game_display,
                        'Date': format_display_date(game.get('date', ''))
                    }
        ...
        with tabs[4]:
            ...
            if view_type == "Season Total":
                ...
                if not standings_df.empty:
                    display_df = standings_df[['Name', 'Wins', 'Losses', 'Confidence']].rename(columns={'Confidence': 'Conf Pts'})
                    table_height = 38 + len(display_df) * 35 + 2
                    st.dataframe(display_df, use_container_width=True, hide_index=False, height=table_height)
                else:
                ...
                if weekly_winners_data:
                    ...
                    table_height = 38 + len(winners_df) * 35 + 2
                    st.dataframe(winners_df, use_container_width=True, hide_index=True, height=table_height)
                else:
                ...
                if has_any_results:
                    ...
                    if week_standings:
                        ...
                        table_height = 38 + len(week_df) * 35 + 2
                        st.dataframe(week_df, use_container_width=True, hide_index=False, height=table_height)
                    else:
        ...
        with tabs[7]:
            ...
            current_s_info = data['seasons'].get(current_season, {})
            is_current_locked = current_s_info.get('locked', False)
            
            if not is_current_locked:
                st.subheader("✅ Finalize Current Season")
                st.info(f"Once all 16 weeks are marked complete, lock **{current_season}** to preserve the final standings and record the champion.")
                weeks_done = sum(1 for w in current_s_info.get('weeks', {}).values() if w.get('winners_set'))
                st.write(f"Weeks completed: **{weeks_done}/16**")
                if weeks_done == 16:
                    if st.button("🔒 Lock & Finalize Season", type="primary", key="finalize_season"):
                        # Compute champion
                        try:
                            s_df = get_season_standings(data, current_season)
                            if not s_df.empty:
                                data['seasons'][current_season]['champion'] = s_df.iloc[0]['Name']
                        except:
                            pass
                        data['seasons'][current_season]['locked'] = True
                        save_data(data)
                        st.success(f"✅ Season {current_season} has been locked and finalized!")
                        st.rerun()
                else:
                    st.warning(f"⚠️ {16 - weeks_done} week(s) still need winners marked before you can finalize.")
            else:
                champion = current_s_info.get('champion', 'Unknown')
                st.success(f"🔒 **{current_season}** is finalized. Champion: **{champion}**")
                if st.button("🔓 Unlock Season (Admin Override)", key="unlock_current"):
                    data['seasons'][current_season]['locked'] = False
                    save_data(data)
                    st.rerun()
            ...
            upcoming_toggle = st.checkbox("Mark as upcoming (no participants yet)", value=True)
            if st.button("Create Season"):
                if new_season_name and new_season_name not in data['seasons']:
                    data['seasons'][new_season_name] = {
                        ...
                        'weeks': {},
                        'locked': False,
                        'upcoming': upcoming_toggle
                    }
                    save_data(data)
                    st.success(f"✅ Created season: {new_season_name}{' (marked as upcoming)' if upcoming_toggle else ''}")
                    st.rerun()
                elif new_season_name in data['seasons']:
                    st.warning("That season already exists.")
            
            st.write("---")
            
            # Manage seasons
            st.subheader("All Seasons")
            for season_name, season_info in sorted(data['seasons'].items(), reverse=True):
                is_upcoming = season_info.get('upcoming', False)
                label = f"**{season_name}**"
                if season_info.get('active'):
                    label += " 🟢 (Active)"
                if season_info.get('locked'):
                    label += " 🔒"
                if is_upcoming:
                    label += " 📅 (Upcoming)"
                with st.expander(label):
                    ...
                    with col1:
                        if not season_info.get('active') and not is_upcoming:
                    ...
                    if is_upcoming:
                        if st.button("Mark as Active (season starting)", key=f"start_upcoming_{season_name}"):
                            data['seasons'][season_name]['upcoming'] = False
                            for s in data['seasons']:
                                data['seasons'][s]['active'] = False
                            data['seasons'][season_name]['active'] = True
                            data['current_season'] = season_name
                    ...
                    champion = season_info.get('champion')
                    if champion:
        ...
        with tabs[9]:
            ...
            # CRITICAL FIX: Force a fresh read from disk to ensure we get the absolute latest data
            # This prevents downloading stale cached data
            try:
                with open(st.session_state.data_file, 'r') as f:
                    fresh_data = json.load(f)
                data_json = json.dumps(fresh_data, indent=2)
                
                # Show a preview of what's in the backup
                week16_games = len(fresh_data.get('seasons', {}).get('2025-2026', {}).get('weeks', {}).get('16', {}).get('games', []))
                week16_picks_count = sum(1 for u in fresh_data.get('users', {}).values() if '16' in u.get('picks', {}))
                st.caption(f"📊 Backup contains: Week 16 has {week16_games} games, {week16_picks_count} users with picks")
                
            except Exception as e:
                st.error(f"Error loading data for backup: {e}")
                data_json = json.dumps(data, indent=2)  # Fallback to in-memory data
    ...
    # Participant view
        ...
        with tabs[1]:
            ...
            # Helper function to find the first week with games where user hasn't submitted picks
            def get_default_week():
                # First pass: find earliest week with games where user hasn't submitted
                for week in range(1, 17):
                    if str(week) not in season_data.get('weeks', {}):
                        continue
                    week_data = season_data['weeks'][str(week)]
                    # Skip weeks without games
                    if not week_data.get('games'):
                        continue
                    # If user hasn't submitted picks for this week, default to it
                    if str(week) not in user_info.get('picks', {}):
                        return week
                    # If user submitted but picks aren't locked yet (no 'submitted' timestamp), default to it
                    if 'submitted' not in user_info.get('picks', {}).get(str(week), {}):
                        return week
                
                # Fallback: if all weeks are submitted, default to latest week with games
                for week in range(16, 0, -1):
                    if str(week) in season_data.get('weeks', {}) and season_data['weeks'][str(week)].get('games'):
                        return week
            ...
            if str(week_num) not in season_data.get('weeks', {}):
                ...
                if not week_data.get('games'):
                    ...
                    if picks_submitted:
                        ...
                        # Track confidence picks across games
                        confidence_picks = []
...
# Footer - Add at the very bottom of the app (dynamic season)
footer_season = data['settings'].get('current_season', '2025-2026')
st.markdown(f"""
<div style='text-align: center; padding: 30px 20px 20px 20px; margin-top: 60px; border-top: 1px solid rgba(128,128,128,0.3); color: #888; font-size: 0.9em;'>
    🏀 {footer_season} Season • College Basketball Pick'Em
</div>
""", unsafe_allow_html=True)
