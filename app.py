...
if not st.session_state.logged_in:
    ...
    with st.sidebar:
        ...
        # Single Past Champions button
        st.write("---")
        if st.button("🏆 Past Champions", use_container_width=True, key="past_champs_btn"):
    ...
    # Past Champions page — shown when clicking the sidebar button
    if st.session_state.get('show_champion_standings'):
        st.markdown("## 🏆 Past Champions")
        st.write("---")

        finished_seasons = sorted(
        ...
        if not finished_seasons:
            ...
            for season_name in finished_seasons:
                s_info = data['seasons'][season_name]
                champion = s_info.get('champion')
                if not champion:
                    try:
                        s_df = get_season_standings(data, season_name)
                        if not s_df.empty:
                            champion = s_df.iloc[0]['Name']
                    except:
                        champion = None

                with st.expander(f"🥇 {season_name}  —  Champion: {champion or 'Unknown'}", expanded=True):
                    season_df = get_season_standings(data, season_name)
                    if not season_df.empty:
                        display_df = season_df[['Name', 'Wins', 'Losses', 'Confidence']].rename(columns={'Confidence': 'Conf Pts'})
        ...
        if st.button("↩️ Back", key="champ_page_back"):
            st.session_state.show_champion_standings = False
            st.rerun()
        st.stop()
