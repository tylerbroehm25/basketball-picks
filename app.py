...
def format_game_display(game, use_markdown=True):
    ...
    if neutral:
        if use_markdown:
            # For streamlit, we'll append a small caption
            return f"{away} @ {home}", True
        else:
            # For dataframes/plain text
...
if not st.session_state.logged_in:
    ...
    if st.session_state.user_type == "admin" or st.session_state.is_admin:
        ...
        with tabs[3]:
            ...
            def get_latest_winners_week():
                ...
                for week in range(1, 17):
                    if str(week) in season_data.get('weeks', {}):
                        if season_data['weeks'][str(week)].get('winners_set'):
                            latest = week
                return latest
        ...
        with tabs[6]:
            ...
            if stat_view == "Personal Stats":
                ...
                def normalize_team_name(name):
                    ...
                    # Normalize common variations and fix misspellings
                    replacements = {
                    ...
                    # Use whole-word matching to avoid double replacements
                    for old, new in replacements.items():
                        # Only replace if it's an exact match for the whole name
                        # or replace as whole word (not mid-word)
                        if name == old:
                            name = new
                            break
        ...
        with tabs[9]:
            ...
            **Season Standings Tiebreaker**
            - Most total wins across all 16 weeks
            ...
            **Season**
            - 16 weeks per season
            - Season standings based on total wins across all weeks
            - Total confidence points accumulated across all 16 weeks serve as the tiebreaker for season standings
    ...
    # Participant view
        ...
        def get_default_week():
            for week in range(1, 17):
                ...
                if str(week) not in user_info.get('picks', {}):
                    return week
                # Check if any games are still unlocked
                if week_data.get('games'):
                    for game in week_data['games']:
                        if not check_game_locked(game.get('date'), data['settings']):
                            return week
            return 1  # Default to week 1 if all locked
        
        tabs = st.tabs(["📋 Make Picks", "🏆 Standings", "📊 My Results", "👁️ View Picks", "📈 Statistics", "📋 All Picks", "👤 My Account", "📋 Rules"])
        ...
        with tabs[4]:
            ...
            else:  # Team Performance - same code as admin
                st.subheader("Performance by Team")
                
                # Use the same team normalization and tracking code from admin section
                # (Copying the team performance logic from admin Statistics tab)
                ...
                for week in range(1, 17):
                    ...
                    for game in week_data.get('games', []):
                        ...
                        for username, u_info in data['users'].items():
                            ...
                            user_picks = u_info.get('picks', {}).get(str(week), {}).get('picks', {})
                            pick = user_picks.get(str(game_id)) or user_picks.get(game_id)
                    ...
                    if table_data:
                        ...
                        for username, u_info in data['users'].items():
                            ...
                            if week_data.get('winners_set'):
                                wins, conf = calculate_week_results(season_data, week_num, username, u_info)
                                losses = 20 - wins
                                summary[u_info['display_name']] = f"{wins}-{losses} ({conf} Confidence Points)"
                                summary_vertical.append({
                                    'Participant': u_info['display_name'],
                                    'Record': f"{wins}-{losses}",
                                    'Confidence Points': conf
